...
# Copyright (C) 2022-2024 Mar Yvan D.

# Dependency Preparation
make clean mrproper
git clean -fdx; git reset --hard
rm -rf AnyKernel/{*.zip,Image.gz-dtb}
git clone --depth=1 https://github.com/mcdofrenchfreis/AnyKernel3.git -b beryllium AnyKernel
...
# Naming Variables
export REVISION=$(cat biofrost-version)
MIN_HEAD=$(git rev-parse HEAD | cut -c 1-12)
export ZIP_NAME="Biofrost-${REVISION}-${MIN_HEAD}"

# GitHub Variables
export COMMIT_HASH=$(git rev-parse --short HEAD)
export BRANCH_NAME=$(git rev-parse --abbrev-ref HEAD)
export REPO_URL="https://github.com/mcdofrenchfreis/biofrost-beryllium"
...
export DEVICE="Pocophone F1"
export CODENAME="Beryllium"
...
function finerr() {
    curl -s -X POST "https://api.telegram.org/bot${BOT_ID}/sendMessage" \
        ...
        -d "disable_web_page_preview=true" \
        -d "parse_mode=html" \
        -d text="<b>Compilation failed, please check build logs for errors.</b>"
    exit 1
}
function compile() {
